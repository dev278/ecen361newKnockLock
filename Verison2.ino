...
ECEN 361; BYU-Idaho
Spring 2018
Zachary Brimhall, Caleb Lamb, and David Varner
*****************************************************************************/ 
#include <Servo.h>
...
// Tuning constants
const int threshold = 60;               // Minimum signal from the piezo to register as a knock
...
Servo myservo;                          // Defining name of servo.
...
void setup() 
{
  ...
  myservo.attach(3);                    //Assigning servo to pin 3
  myservo.write(90);                    //Starting position is in the locked position at beginning of program.
  
  Serial.begin(9600);
  Serial.println("Program start.");
  
  digitalWrite(greenLED, HIGH);         // Green LED on, program is ready to run.
}
...
void loop()                             //Main loop for program.
{
  ...
  {
    programButtonPressed = true;        // Yes and save state.
    digitalWrite(redLED, HIGH);         // Turn on the red light to let user know we're programming.
  } 
  else 
  {
  ...
  if (knockSensorValue >=threshold)     //If the sensor hears a knock....
  {
    listenToSecretKnock();              //Start listening function.
  }
